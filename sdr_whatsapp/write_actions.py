"""Script auxiliar para reescrever actions.ts com as correções do kanban e métricas."""
import os
...
target = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'sdr-hub', 'app', 'actions.ts')
with open(target, 'w', encoding='utf-8') as f:
    f.write(content)

print(f"actions.ts reescrito: {len(content)} chars, {len(content.splitlines())} linhas")
